---
                {
  "source": "web",
  "label": "man7-user_namespaces",
  "url": "https://man7.org/linux/man-pages/man7/user_namespaces.7.html",
  "description": "User namespaces fundamentals (rootless containers, UID/GID mappings, isolation)",
  "license": null,
  "collected_at": "2025-12-15T17:15:23.759253+00:00"
}
                ---
...
user_namespaces(7) — Linux manual page
NAME | DESCRIPTION | STANDARDS | NOTES | EXAMPLES | SEE ALSO | COLOPHON
```
...
```
   ...
   Nested namespaces, namespace membership
       User namespaces can be nested; that is, each user namespaceâexcept
       the initial ("root") namespaceâhas a parent user namespace, and
   ...
   Capabilities
       ...
       â¢  A process has a capability inside a user namespace if it is a
       ...
       â¢  If a process has a capability in a user namespace, then it has
          that capability in all child (and further removed descendant)
          namespaces as well.
       â¢  When a user namespace is created, the kernel records the
   ...
   Effect of capabilities within a user namespace
       ...
       mounts and mount the following types of filesystems:
           â¢  /proc (since Linux 3.8)
           â¢  /sys (since Linux 3.8)
           â¢  devpts (since Linux 3.9)
           â¢  tmpfs(5) (since Linux 3.9)
           â¢  ramfs (since Linux 3.9)
           â¢  mqueue (since Linux 3.9)
           â¢  bpf (since Linux 4.4)
           â¢  overlayfs (since Linux 5.11)
       Holding CAP_SYS_ADMIN within the user namespace that owns a
   ...
   User and group ID mappings: uid_map and gid_map
       ...
       (3)  The size of the range of user IDs that is mapped between the
            two user namespaces.
       System calls that return user IDs (group IDs)âfor example,
       getuid(2), getgid(2), and the credential fields in the structure
       returned by stat(2)âreturn the user ID (group ID) mapped into the
   ...
   Defining user and group ID mappings: writing to uid_map and gid_map
       ...
       â¢  The three fields must be valid numbers, and the last field must
          be greater than 0.
       â¢  Lines are terminated by newline characters.
       â¢  There is a limit on the number of lines in the file.  In Linux
       ...
       â¢  The range of user IDs (group IDs) specified in each line cannot
          ...
          maps.
       â¢  At least one line must be written to the file.
       ...
       â¢  The writing process must have the CAP_SETUID (CAP_SETGID)
          capability in the user namespace of the process pid.
       â¢  The writing process must either be in the user namespace of the
          process pid or be in the parent user namespace of the process
          pid.
       â¢  The mapped user IDs (group IDs) must in turn have a mapping in
          the parent user namespace.
       â¢  If updating /proc/pid/uid_map to create a mapping that maps UID
       ...
       â¢  One of the following two cases applies:
          ...
          (b)  Or otherwise all of the following restrictions apply:
               ...
               â¢  The writing process must have the same effective user
                  ID as the process that created the user namespace.
               â¢  In the case of gid_map, use of the setgroups(2) system
   ...
   Project ID mappings: projid_map
       ...
       â¢  The writing process must either be in the user namespace of the
          process pid or be in the parent user namespace of the process
          pid.
       â¢  The mapped project IDs must in turn have a mapping in the
          parent user namespace.
   ...
   Operation of file-related capabilities
       ...
       â¢  the process has the relevant effective capability in its user
          namespace; and
       â¢  the file's user ID and group ID both have valid mappings in the
          user namespace.
