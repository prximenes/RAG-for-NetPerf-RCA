---
                {
  "source": "web",
  "label": "kernel-network-scaling",
  "url": "https://www.kernel.org/doc/html/latest/networking/scaling.html",
  "description": "RPS/RFS/XPS and scaling topics relevant to softirq/IRQ issues",
  "license": null,
  "collected_at": "2025-12-15T17:15:16.314137+00:00"
}
                ---
...
affinity of each interrupt see SMP IRQ affinity. Some systems
...
a driver sends a packet up the network stack with
```
netif_rx()
```
 or
...
CPU. SMP IRQ affinity explains how CPUs are assigned to
the bitmap.
Suggested Configuration¶
...
CPU for packet processing (from
```
get_rps_cpu()
```
) the rps_sock_flow table
...
management to the Kernel by calling
```
...
transmitting the first packet in a flow, the function
```
get_xps_queue()
```
 is
...
get_xps_queues()
```
 over all packets in the flow. To avoid
